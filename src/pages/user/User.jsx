...
export default function User() {
    ...
    const deleteStuff = () => {
        instance.delete(`user/${currentItemToDelete.id}`)
        .then(res => {
    ...
    return (
        <Case>
                                ...
                                {stuffs.map((user, id) => (
                                    <tr key={user.id} className="border-b dark:border-neutral-500">
                                        ...
                                        <td className="whitespace-nowrap px-6 py-4">
                                            <Link to={'/user/edit/' + user.id} className="px-4 py-2 bg-orange-500 rounded-lg mr-2 font-bold text-black">Edit</Link>
                                            <button type="button" onClick={() => handleDeleteClick(user)} className="px-4 py-2 bg-red-500 rounded-lg mr-2 font-bold text-black">Hapus</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
            ...
            <DeleteModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={deleteStuff} itemName={currentItemToDelete ? currentItemToDelete.username : ''} />
            <ToastContainer />
        </Case>
    );
}
